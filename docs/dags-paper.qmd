...
subtitle: Adapting qualitative and quantative research methods for better inferences about existing arguments and evidence
...
<!-- generalizing --> 
Literature reviews inevitably involve generalizing. Summarizing and diagramming literature is an excercise in generalizing concepts (boxes, "nodes") and generalizing relationships (arrows, "edges").
To construct a diagram representing prior work,  we must choose to assign the key concepts of prior studies to either the same box or different boxes or nodes. Likewise, we must decide if a given study theorized or tested a given causal relationship. The process of choosing prior studies to review and assigning labels to the concepts and relationships they discuss is parallel to what we do when we choose cases to study, the what @Soss2021 calls "casing."
...
[ADD MORE ON PARALLELS WITH SOSS, FRICKER, RIOFRANCOS, ETC.]
...
filter(edges, cites == "Ziblatt 2009")  |>
  netlit_plot()
  
filter(edges, cites == "Beach and Petersen 2013")  |>
  netlit_plot()
```
